...
export class Shooting extends ActivityState {
  ...
  public enter(previous: ActivityState, action: ShootAction) {
    super.enter(previous, action);
    this.position = action.position;
  }

  public leave(_next: ActivityState, action: ShootAction): void | Promise<void> {
    ...
    const hit = this.world.addEntities({ type: 'shoot-hit', position: this.position });
  ...
  public update(deltaTime: number): void | boolean {
    ...
    if (typeof super.update(deltaTime * combatant.shootSpeed) === 'boolean') {
