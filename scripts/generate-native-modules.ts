...
// example:
// import {} from '@aupeasy-lib/peasy-ui'
// ->
// import {} from '../../peasy-ui/dist/esm/index.js'
        ...
        switch (statement.kind) {
          ...
          case ts.SyntaxKind.ExportDeclaration: {
            if (typeof statement.moduleSpecifier?.text === 'string') {
              ...
              if (specifier.startsWith('@peasy-lib')) {
                const start = statement.moduleSpecifier.getStart(sourceFile);
                const end = statement.moduleSpecifier.getEnd(sourceFile);

                const packageName = specifier.slice('@peasy-lib/'.length);
                const packageEntryFilePath = path.join(project.path, `node_modules/@peasy-lib/${packageName}/dist/native-modules/index.mjs`);
                const normPath = file.path.replace(/\\/g, '/');
                const origin = pkg.name.npm.includes('@peasy-lib')
                  ? normPath.replace(/packages[\\\/]([\w-]+)/, 'node_modules/@peasy-lib/$1')
