...
export class Vector {
  ...
  public get zeroish(): boolean {
    return Math.abs(this.x) < Vector.Epsilon && Math.abs(this.y) < Vector.Epsilon;
  }
  ...
  public abs(update = false): Vector {
    const vector = update ? this : this.clone();
    vector.x = this.x < 0 ? -this.x : this.x;
    vector.y = this.y < 0 ? -this.y : this.y;
    return vector;
  }
